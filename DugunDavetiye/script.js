/* ============================================================================
   EDA & EMİRHAN — DİJİTAL DÜĞÜN DAVETİYESİ
   script.js
   ----------------------------------------------------------------------------
...
const CONFIG = {
  ...
  // Örn: 30 Mayıs 2026, saat 20:00 -> "2026-05-30T20:00:00"
  dugunTarihiISO: "2026-05-30T20:00:00",

  // Ekranda gösterilecek tarih/saat metinleri (serbestçe yazabilirsin)
  tarihBuyukMetin: "30 MAYIS 2026",   // Kapak bölümünde
  tarihKisaMetin:  "30.05.2026",      // Mekan kartında
  saatMetin:       "20:00",

  /* --- MEKAN -------------------------------------------------------------- */
  mekanAdi:  "Çetinkaya Fabrika Garden Kır Düğün Salonu",
  ...
  haritaEmbedUrl: "https://www.google.com/maps?q=%C3%87anakkale+Merkez&output=embed",

  // "Haritada görmek için tıklayınız" linkinin gideceği normal Google Maps adresi.
  haritaLinki: "https://www.google.com/maps/search/?api=1&query=Çetinkaya+Fabrika+Garden+Kır+Düğün+Salonu+Çanakkale",

  /* --- AİLELER ------------------------------------------------------------ */
  gelinAilesi: "Elif & Ferit",        // Gelinin anne-babası
  damatAilesi: "Selma & Aydın",       // Damadın anne-babası
  ...
  /* --- NOT BÖLÜMÜ --------------------------------------------------------- */
  notMetni: "Davetimiz yemekli olup, takı töreni girişte yapılacaktır.",

  /* --- RSVP (KATILIM BİLDİRİMİ) ------------------------------------------ */
  rsvpSonTarih: "20 MAYIS 2026",
...
function rsvpKur() {
  ...
  function whatsappLinkiUret() {
    ...
    const mesaj =
      `Düğün Katılım Bildirimi%0A` +
      `Çift: ${CONFIG.gelinAdi} & ${CONFIG.damatAdi}%0A` +
      `İsim: ${isim}%0A` +
      `Durum: ${katilim}%0A` +
      `Kişi Sayısı: ${kisi}`;

    return `https://wa.me/${CONFIG.whatsappNumarasi}?text=${mesaj}`;
  }
